...
public class LeetCode8 {
    ...
    public int myAtoi(String s) {
        ...
        int index = 0;
        List<Character> characters = Arrays.asList('1', '2', '3', '4', '5', '6', '7', '8', '9', '0');
        StringBuilder res = new StringBuilder();
        while (index<trim.length()){

            if(characters.contains(trim.charAt(index))){
                res.append(trim.charAt(index));
                index++;
            }else {
                break;
            }
        ...
        int target;
        try {
            target=Integer.valueOf(res.toString());
        }catch (Exception e){
            if (isFuShu){
    ...
    /**
     * 通过除以10判断当前数值是否越界当前数值越界
     * @param s
     * @return
     */
    public int heAtoi(String s) {
        if(s.equals("")){
            return 0;
        }
